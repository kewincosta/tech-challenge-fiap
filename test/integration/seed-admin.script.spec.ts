...
describe('seed-admin script', () => {
  it(
    'creates a SUPER_ADMIN with the validated document when given a valid one',
    async () => {
  ...
  it(
    'refuses to run without a valid document and creates nobody',
    async () => {
      const email = uniqueEmail();
  ...
  it(
    'leaves exactly one super administrator when run twice',
    async () => {
      const email = uniqueEmail();
      const overrides = {
        ADMIN_EMAIL: email,
