...
| `InventoryItem.restore` deliberately attaches no movements, so nothing in the aggregate can see prior consumptions. | `inventory-item.ts:96` | The return path cannot find the consumption to point at from the aggregate | The work-orders side already knows the movement ids: `ConsumeStockBatchHandler` returns them and the return path passes the target back in. No change to the aggregate's loading rule. |
...
| Where the movement id for a return comes from | Returned by `ConsumeStockBatchHandler` and passed back on the return call | Keeps `InventoryItem.restore`'s no-movements rule intact, and makes `undoes_movement_id` provably correct rather than inferred. |
