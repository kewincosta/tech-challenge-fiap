...
import { ReturnExceedsWithdrawnError } from '../../../domain/errors/return-exceeds-withdrawn.error';
...
import { ReturnPartsHandler } from './return-parts.handler';
...
const WITHDRAWN_ITEM_ID = WorkOrderItemId.create('66666666-6666-4666-8666-666666666666');
const WITHDRAWN_INVENTORY_ITEM_ID = '77777777-7777-4777-8777-777777777777';

function withdrawnPart(withdrawnQuantity = 2): WorkOrderPartItem {
  return WorkOrderPartItem.restore({
    id: WITHDRAWN_ITEM_ID,
    inventoryItemId: WITHDRAWN_INVENTORY_ITEM_ID,
    sku: 'FLT-001',
...
function makeHandler(commandBusExecute?: (command: unknown) => Promise<unknown>) {
  ...
  const handler = new ReturnPartsHandler(
    workOrders,
    transactionRunner,
    new FakeClock(),
    commandBus.bus,
...
describe('ReturnPartsHandler', () => {
  ...
  it('lets ReturnExceedsWithdrawnError travel out untouched, before any transaction', async () => {
    const { handler, workOrders, transactionRunner } = makeHandler();
  ...
  it('opens exactly one transaction, saves the work order, then dispatches RestoreStockBatchCommand with plain lines', async () => {
    ...
    expect(updated.partItems[0].withdrawnQuantity).toBe(0);
  ...
  it('lets an error raised by the inventory side travel out untouched', async () => {
    const { handler, workOrders } = makeHandler(() =>
      Promise.reject(new Error('no pending consumption covers this return')),
    );
    await workOrders.save(restoreInExecution());
