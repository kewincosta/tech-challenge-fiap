...
import { LogoutAllSessionsCommand, LogoutAllSessionsResultDto } from './logout-all-sessions.command';

@CommandHandler(LogoutAllSessionsCommand)
export class LogoutAllSessionsHandler
  implements ICommandHandler<LogoutAllSessionsCommand, LogoutAllSessionsResultDto>
{
  constructor(
  ...
  async execute(command: LogoutAllSessionsCommand): Promise<LogoutAllSessionsResultDto> {
    ...
    if (revokedIds.length > 0) {
      ...
      this.eventBus.publish(
        new AllUserSessionsRevoked(command.userId, revokedIds, SessionRevocationReason.LogoutAll, now),
      );
    }
